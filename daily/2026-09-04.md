...
明确披露 AI 协作：GPT-5.5 Pro 参与搜索、化简并检查二十维 stealth 构型及其与 Vardy 堆积的形式对偶关系，作者随后核验计算；模型还提供编辑反馈。
...
明确披露 AI 协作：作者披露主要使用 ChatGPT 等 AI 工具；定理和证明均由作者撰写并核验，作者承担全部责任。
...
明确披露 AI 协作：作者使用 ChatGPT 5.6 Sol 解释相关文献与工具，包括 Blaschke 选择和实解析曲线选择引理；论证由作者独立发展和验证。
...
明确披露 AI 协作：大型语言模型建议把 Chodosh–Gianocca 方法用于指标估计并使用 Spin(7) 分解；作者主导、重写并独立验证全部数学内容。
...
明确披露 AI 协作：作者披露反例构造通过 ChatGPT Plus 实验发现；模型还生成初稿并参与校对，作者独立检查全部计算与论证。
...
明确披露 AI 协作：作者使用 ChatGPT 协助表述、组织、制图、校对以及论证呈现/检查建议，并独立验证所有贡献。
...
明确披露 AI 协作：作者使用大型语言模型协助代数核对、部分显式计算和语言编辑，并说明独立检查全部数学论证。
...
明确披露 AI 协作：作者使用 ChatGPT 5.6 辅助概念形成和计算；数学验证、最终证明决策与文字由作者负责。
...
明确披露 AI 协作：ChatGPT Sol 5.6 用于终稿校对和检查公式一致性；作者明确说明未用于发展数学结果、算法或例子。
...
明确披露 AI 协作：ChatGPT 用于终稿校对并发现少量错误与遗漏；作者承担剩余错误责任。
